...
class MongoStepLogger extends BaseStepLogger {
    ...
    async getRaw() {
        const where = { 'slot': `steps.${this.name}.logs`, ...this.getFilter() };
        const sort = { 'time': 1 };
        return new Promise((resolve, reject) => {
            this.db.collection(MongoHelper.getCollection('logs')).find(where, { sort })
                .toArray((err, docs) => {
                    if (err) {
                        reject(err);
                    }
    ...
    _reportLog(message, syncId) {
        const key = 'logs';
        this.db.collection(MongoHelper.getCollection(key)).insertOne(this.getObjectToPush(key, message, syncId), (err) => {
            if (err) {
    ...
    _reportOutputUrl() {
        ...
        this.db.collection(MongoHelper.getCollection(key)).updateOne(
            this.getFilter(),
            { $set: { [key]: this.outputUrl } },
            { upsert: true },
            (err) => {
                if (err) {
                    this.emitter.emit('ERROR', err);
    ...
    _reportPrevioulyExecuted() {
        ...
        this.db.collection(MongoHelper.getCollection(key)).updateOne(
            this.getFilter(),
            { $set: { [key]: this.previouslyExecuted } },
            { upsert: true },
            (err) => {
                if (err) {
                    this.emitter.emit('ERROR', err);
    ...
    async _reportStatus() {
        ...
        return this.db.collection(MongoHelper.getCollection(key)).updateOne(
            this.getFilter(),
            { $set: { [key]: this.status } },
            { upsert: true },
            (err) => {
    ...
    _reportCreationTimestamp() {
        ...
        this.db.collection(MongoHelper.getCollection(key)).updateOne(
            this.getFilter(),
            { $set: { [key]: this.creationTimeStamp } },
            { upsert: true },
            (err) => {
                if (err) {
                    this.emitter.emit('ERROR', err);
    ...
    _reportMemoryUsage(time, memoryUsage, syncId) {
        ...
        this.db.collection(MongoHelper.getCollection(key)).insertOne(this.getObjectToPush(key, { time, usage: memoryUsage }, syncId), (err) => {
            if (err) {
                this.emitter.emit('ERROR', err);
            }
        });
    ...
    _reportCpuUsage(time, cpuUsage, syncId) {
        ...
        this.db.collection(MongoHelper.getCollection(key)).insertOne(this.getObjectToPush(key, { time, usage: cpuUsage }, syncId), (err) => {
            if (err) {
                this.emitter.emit('ERROR', err);
            }
        });
    ...
    _reportStepProgress() {
        const key = `steps.${this.name}.progress`;
        this.db.collection(MongoHelper.getCollection(key)).updateOne(
            this.getFilter(),
            { $set: { [key]: this.stepProgress } },
            { upsert: true },
            (err) => {
                if (err) {
                    this.emitter.emit('ERROR', err);
